...
export default function Dashboard() {
  ...
  const userName = "Athlete"; // Real apps would get this from state/auth
      ...
      <header className="fixed top-0 w-full max-w-[430px] left-1/2 -translate-x-1/2 z-50 bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl flex justify-between items-center px-6 py-4 border-b border-outline-variant/5">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/profile')} className="w-10 h-10 rounded-full overflow-hidden bg-surface-container-high transition-transform active:scale-95 cursor-pointer border-none p-0 outline-none ring-2 ring-primary/10">
            <img alt="User Profile Avatar" className="w-full h-full object-cover" src="https://lh3.googleusercontent.com/aida-public/AB6AXuDt4nHXSVQB2_KC0xGNVWAcqHwkY_ckUSTQTZF6rLF1Haxs3GqvSae9uqOpOOknxcfrJ7CjHUv7tk9ea1sIuAh8kAwuPGh-oM0NcZZQ-gSzQanrEU5vW7ptNblJQhYhIe0-uy6K-hjborR0m8eZXo4SutxE5dzeWd-eLitn4VCjR0kYxLuVvP6yoogcu9_FQJwUlIK97PjC3PjG6n0FTZW4EILe4ZfOlQxvuRLmhySjnuK4YngE6sx6zOLYTcJiSDFax3AZiOndS_Q"/>
          </button>
          <div>
            <p className="text-[0.6rem] font-bold uppercase tracking-widest text-secondary font-label leading-none">Aura Peak</p>
      ...
      <main className="mt-24 px-6 max-w-[430px] mx-auto space-y-10">
        ...
        <section>
          <div className="flex justify-between items-end mb-6">
            ...
            <span className="text-tertiary font-bold font-label text-[0.6rem] uppercase tracking-widest">14 DAY STREAK</span>
          </div>
            ...
            {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((day, i) => {
              ...
              return (
                <div key={i} className="flex flex-col items-center gap-2">
                  <span className={`text-[0.6rem] font-bold font-label ${isToday ? 'text-secondary' : 'text-on-surface-variant'}`}>{day}</span>
                  <div className={`w-10 h-14 rounded-full flex items-center justify-center transition-all ${
                    isToday ? 'bg-secondary text-white shadow-lg shadow-secondary/20 ring-2 ring-secondary/20 ring-offset-2' : 
                    isPast ? 'vitality-gradient text-white shadow-sm' : 'bg-surface-container-low border border-outline-variant/10'
                  }`}>
                    {isPast ? (
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          >
            <div className="z-10">
              <p className="text-[0.6rem] font-bold font-label uppercase tracking-widest text-secondary mb-1">Coach Level</p>
          ...
          <div className="col-span-1 flex flex-col gap-4">
            <div className="p-5 rounded-2xl bg-surface-container-low flex flex-col justify-between h-[calc(50%-8px)] border border-outline-variant/5 shadow-sm">
               ...
               <p className="font-headline font-bold text-xl">12.4k</p>
            </div>
            <div className="p-5 rounded-2xl bg-surface-container-low flex flex-col justify-between h-[calc(50%-8px)] border border-outline-variant/5 shadow-sm">
               <div className="flex justify-between items-start">
               ...
               <p className="font-headline font-bold text-xl">420<span className="text-xs ml-0.5">kcal</span></p>
            </div>
          </div>
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div>
                <p className="font-headline text-md font-extrabold text-on-surface">Precision Insight</p>
                <p className="text-on-surface-variant text-sm mt-1 leading-relaxed">Your metabolic rate is <span className="text-secondary font-bold">12% higher</span> in evening sessions. Today's optimal window: <span className="text-secondary font-bold">6:30 PM</span>.</p>
              </div>
            </div>
          </motion.div>
        </div>
        ...
        <section>
          ...
          <div className="flex flex-col gap-6">
            {[
              { id: 1, type: 'Strength', title: 'Upper Body Power', time: '45 mins', level: 'Intermediate', img: 'https://images.unsplash.com/photo-1541534741688-6078c6bfb5c5?q=80&w=1469&auto=format&fit=crop' },
              { id: 2, type: 'Cardio', title: 'Evening Tempo Run', time: '5.2 km', level: 'Moderate', img: 'https://images.unsplash.com/photo-1534438327276-14e5300c3a48?q=80&w=1470&auto=format&fit=crop' }
            ].map((workout) => (
              <motion.div 
                whileTap={{ scale: 0.98 }}
                key={workout.id}
              ...
              >
                <img alt={workout.title} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" src={workout.img}/>
                ...
                <div className="relative z-10 flex justify-between items-end">
                  <div>
                    ...
                    <h3 className="text-white font-headline text-2xl font-bold tracking-tight">{workout.title}</h3>
                    <p className="text-white/60 text-xs mt-1 font-medium">{workout.time} • {workout.level}</p>
                  </div>
